...
def get_cmd_args():
    ...
    if os.path.basename(sys.argv[0]) == 'flaskerizer' or os.path.basename(sys.argv[0]) == '__main__.py':
        ...
        parser.add_argument('--templates-path', type=str, default=os.path.dirname(example.__file__), dest='templates_path')

        parser.add_argument('--app-name', type=str, default='Test_application', dest='app_name')

        parser.add_argument('--app-path', type=str, default=os.path.dirname(flaskerizer.__file__), dest='app_path')
