...
def get_cmd_args():
    ...
    if os.path.basename(sys.argv[0]) == 'flaskerizer' or os.path.basename(sys.argv[0]) == '__main__.py':
        ...
        parser.add_argument('--top-path', type=str, required=True, dest='top_level_path',
                            help='Full path of the top level folder of the Bootstrap template')

        parser.add_argument('--temp-path', type=str, required=True, dest='templates_path',
                            help='Full path of the folder containing the HTML files of the Bootstrap template')

        parser.add_argument('--name', type=str, default='Flaskerized_app', dest='app_name',
                            help="Name of your Flask app, note cannot be named 'app'")

        parser.add_argument('--path', type=str, required=True, dest='app_path',
                            help='Full path of the destination folder for your Flask app')

        parser.add_argument('--large-app', dest='large_app_structure', action='store_true',
                            help='Create a Flask app with large package based structure')

        parser.add_argument('--no-large-app', dest='large_app_structure', action='store_false',
        ...
        if vars(parser.parse_known_args()[0])['app_name'] == 'app':
            print("The value of --name cannot be set to the string 'app'\n"
        ...
        parser.add_argument('--top-path', type=str, default=os.path.dirname(example.__file__), dest='top_level_path')

        parser.add_argument('--temp-path', type=str, default=os.path.dirname(example.__file__), dest='templates_path')

        parser.add_argument('--name', type=str, default='Test_application', dest='app_name')

        parser.add_argument('--path', type=str, default=os.path.dirname(flaskerizer.__file__), dest='app_path')

        parser.add_argument('--large-app', default=True, dest='large_app_structure', action='store_true')
